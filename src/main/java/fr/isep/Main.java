...
public class Main {
    public static void main(String[] args) {
        ...
        // Charger les vols
        List<Vol> vols = readFile.loadVols();
        System.out.println("Vols chargés :");
        for (Vol vol : vols) {
            System.out.println("Vol numéro " + vol.getNumeroVol() + " de " + vol.getOrigine().getNom() + " à " + vol.getDestination().getNom());
        }

        // Charger les passagers
        List<Passager> passagers = readFile.loadPassagers();
        System.out.println("Passagers chargés :");
        for (Passager passager : passagers) {
            passager.obtenirInfos();
        }

        // Charger les pilotes
        List<Pilote> pilotes = readFile.loadPilotes();
